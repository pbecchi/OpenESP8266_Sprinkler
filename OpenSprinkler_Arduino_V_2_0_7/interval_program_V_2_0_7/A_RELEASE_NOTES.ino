...
-------------------------------
Please Note - currently V2.0.7 is being updated to the latest OpenSprinkler code - expect it to be finished in the next few days. 

The V2.0.7 version currently uploaded in this repo has NOT been extensively tested.
...
Version:     Opensprinkler-Arduino 2.0.7
...
- Freetronics LCD Shield from [Freetronics LCD]
    ...
    This is a fairly common 16x2 LCD, however the main difference is the buttons are onboard the shield and are all sampled using 

one analog pin (each button has its own ADC voltage read from pin A0 to indicate which one was pressed). 
...
- hostname isn't implemented for opensprinkler - you'll need to use IP to access (this seems to be a limitation of the Arduino 

Ethernet library

- port number is currently hard coded (default is 80) until I figure out how to change it dynamically when defining the 

EthernetServer object

- the number of discrete pins is pre-defined as 16 maximum - you'll need to edit OpenSprinklerGen2.cpp and defines.h if you want 

more - make sure they're in multiples of 8 
